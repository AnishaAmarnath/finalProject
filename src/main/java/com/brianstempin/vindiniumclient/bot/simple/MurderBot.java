...
public class MurderBot implements SimpleBot {
    ...
    public BotMove move(GameState gameState) {
        ...
        if (runAwayMode == true && gameState.getHero().getGold() >= 2 && gameState.getHero().getLife() <= 80) {
        	...
        	Set<GameState.Hero> Mineheros = new HashSet<>();
            	for(GameState.Hero currentHero : Mineheros) {
            		if(currentHero.getMineCount()<=10){
            			return BotUtils.directionTowards(gameState.getHero().getPos(), currentHero.getPos());
            		}
            		
            		if(currentHero.getLife() <= 20){
                    	logger.info("getting the hero with higher mines");
                    			 return BotUtils.directionTowards(gameState.getHero().getPos(), currentHero.getPos());
            		}
